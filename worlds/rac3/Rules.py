...
def set_rules_hard_location(world):
    ...
    add_rule(world.get_location("Annihilation: Qwarktastic Battle"),
             lambda state: state.has_all([RAC3ITEM.HACKER, RAC3ITEM.TYHRRA_GUISE, RAC3ITEM.HYPERSHOT, RAC3ITEM.GRAV_BOOTS], world.player)
    ...
    # Escape Pod: None
    add_rule(world.get_location("Crash Site: Infobot: Aridia"), lambda state: state.has(RAC3ITEM.GRAV_BOOTS, world.player))
    ...
    if world.options.titanium_bolts.value == 1:
        ...
        add_rule(world.get_location("Daxx: T-Bolt: Time Sensitive Door"),
                 lambda state: state.has_all([RAC3ITEM.HYPERSHOT, RAC3ITEM.CHARGE_BOOTS, RAC3ITEM.HACKER], world.player))
        # Holostar Studios
    ...
    if world.options.skill_points.value > 0:
        ...
        add_rule(world.get_location("Metropolis: Skill Point: 2002 was a good year in the city"),
                 lambda state: state.has_any([RAC3ITEM.FLUX_RIFLE, RAC3ITEM.PROGRESSIVE_FLUX_RIFLE, RAC3ITEM.ANNIHILATOR,
    ...
    if world.options.trophies.value > 1:
        add_rule(world.get_location("Phoenix: Long Term Trophy: Titanium Collector"),
                 lambda state: state.has(RAC3ITEM.PROGRESSIVE_VIDCOMIC, world.player, 5)
                               and state.has_all([RAC3ITEM.REFRACTOR, RAC3ITEM.GRAV_BOOTS, RAC3ITEM.HACKER, RAC3ITEM.HYPERSHOT,
        ...
        add_rule(world.get_location("Phoenix: Long Term Trophy: Annihilation Nation Champion"),
                 lambda state: state.has(RAC3ITEM.PROGRESSIVE_VIDCOMIC, world.player, 5)
                               and state.has_all([RAC3ITEM.HACKER, RAC3ITEM.TYHRRA_GUISE, RAC3ITEM.HYPERSHOT, RAC3ITEM.GRAV_BOOTS],
                                                 world.player))

        add_rule(world.get_location("Phoenix: Long Term Trophy: Skill Master"),
                 lambda state: state.has(RAC3ITEM.PROGRESSIVE_VIDCOMIC, world.player, 5)
                               # Gadgets
                               and state.has_all([RAC3ITEM.GRAV_BOOTS, RAC3ITEM.HACKER, RAC3ITEM.HYPERSHOT, RAC3ITEM.REFRACTOR,
                                                  RAC3ITEM.TYHRRA_GUISE, RAC3ITEM.WARP_PAD], world.player)
                               # Planets
                               and state.can_reach("Florana", player=world.player)
                               and state.can_reach("Starship Phoenix", player=world.player)
                               ...
                               and state.can_reach("Command Center", player=world.player)
                               # Weapons
                               and state.has_all([RAC3ITEM.PLASMA_WHIP, RAC3ITEM.SPITTING_HYDRA, RAC3ITEM.SUCK_CANNON, RAC3ITEM.DISC_BLADE,
...
def set_rules(world: "RaC3World"):
    ...
    # world.multiworld.completion_condition[world.player] = lambda state: state.has(RAC3ITEM.VICTORY,
    # world.player)
